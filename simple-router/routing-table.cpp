...
RoutingTable::lookup(uint32_t ip) const
{
	...
	RoutingTableEntry* temp_entry = NULL;
	while (iter != m_entries.end())
	{
		auto req_mask = ip & (iter->mask);
		auto dest_mask = (iter->dest) & (iter->mask);
		if (req_mask == dest_mask)
			if (temp_entry == NULL || ((iter->mask) > (temp_entry->mask)))
				temp_entry = (RoutingTableEntry*) & (*iter);
		iter++;
	}
	if (temp_entry == NULL) throw std::runtime_error("Routing entry not found");
	else return *temp_entry;
}
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
